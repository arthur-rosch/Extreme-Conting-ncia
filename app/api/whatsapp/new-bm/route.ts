...
// Schema de validação para nova BM
const newBMSchema = z.object({
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validatedData = newBMSchema.parse(body);
    
    const { bmId, name, price, location, description, recipients } = validatedData;
    ...
    // Enviar notificações
    const results = await whatsappServiceSDK.sendNewBMNotification(
      formattedNumbers,
      {
        id: bmId,
        name,
    ...
    console.error('Erro na rota de nova BM:', error);
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      message: 'Rota de nova BM funcionando com SDK',
    });
